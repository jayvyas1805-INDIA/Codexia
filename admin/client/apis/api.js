...
// Base URL for admin dashboard API. Prefer Vite env var `VITE_API_BASE_URL`, fallback to backend port 3000.
...
export const getPostsApi = async (params) => {
  try {
    // try authenticated first (useful for admin-only filters)
    const response = await api.get("/posts", { headers: authHeader(), params });
    return response.data;
  } catch (error) {
    // if unauthorized, try unauthenticated fallback so the dashboard can still show public posts
    if (error?.response?.status === 401) {
      try {
        const res2 = await api.get("/posts", { params });
        return res2.data;
      } catch (err2) {
        throw err2.response?.data || { message: "Fetching posts failed" };
      }
    }
...
export const addModeratorApi = (communityId, data) =>
  api.post(`/communities/${communityId}/add-moderator`, data, {
    headers: authHeader(),
  });
// Create new post
...
export const getCommunitiesApi = async (params) => {
  try {
    const response = await api.get("/communities", { headers: authHeader(), params });
    return response.data;
  } catch (error) {
    if (error?.response?.status === 401) {
      try {
        const res2 = await api.get("/communities", { params });
        return res2.data;
      } catch (err2) {
        throw err2.response?.data || { message: "Fetching communities failed" };
      }
    }
  ...
  try {
    ...
    if (error?.response?.status === 401) {
      try {
        ...
        throw err2.response?.data || { message: "Fetching users failed" };
      }
    }
...
export const getReportsApi = async (params) => {
  try {
    ...
    if (error?.response?.status === 401) {
      try {
        const res2 = await api.get("/reports", { params });
        return res2.data;
      } catch (err2) {
        throw err2.response?.data || { message: "Fetching reports failed" };
      }
    }
...
export const getActivityLogsApi = async (params) => {
  try {
    ...
    if (error?.response?.status === 401) {
      try {
        const res2 = await api.get("/activity", { params });
        return res2.data;
      } catch (err2) {
        throw err2.response?.data || { message: "Fetching activity logs failed" };
      }
    }
